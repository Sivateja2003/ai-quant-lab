...
def _load_kite() -> KiteConnect:
    ...
    if not api_key or api_key == "your_api_key_here":
        sys.exit(
            "ERROR: KITE_API_KEY not set. Copy .env.example to .env and fill it in."
        )
...
def generate_session(request_token: str) -> str:
    ...
    if not api_secret or api_secret == "your_api_secret_here":
        sys.exit(
            "ERROR: KITE_API_SECRET not set. Copy .env.example to .env and fill it in."
        )
...
def get_authenticated_kite() -> KiteConnect:
    ...
    if not access_token:
        sys.exit(
            "ERROR: KITE_ACCESS_TOKEN not set.\n"
